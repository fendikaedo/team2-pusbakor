...
<div class="container font-weight-bold text-dark">
    <a href="/modal/create" class="btn btn-success btn-sm m-3">
    ...
    <tbody class="text-center">
        @foreach ($user as $u)
            <tr>
                ...
                <td>{{ $u->email }}</td>
                <td>
                    <form action="{{ route('admin.destroy', $u->id) }}" method="POST" data-confirm-delete="true">
                        @method('DELETE')
                        @csrf
                        <a href="admin/{{ $u['id'] }}/edit" class="btn btn-info btn-sm"><svg
